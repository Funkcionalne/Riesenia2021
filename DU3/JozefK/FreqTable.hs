...
--ak testovač nič nepovie, tak o efektivite nebola v zadaní reč :) na osvieženie pifko
{-
                        .sssssssss.
                   .sssssssssssssssssss
        ...
        \________ .........................|
               ...
               |.............................|
                  |.......................|
                      |...............|
-}
--funkcia na split z minulej úlohy
--P.S: ja som rodený zátvorkár :)

splitWords  :: (Char -> Bool) -> String -> [String]
splitWords _  "" = [] 
splitWords whiteSpace (x:xs) | (whiteSpace x) = splitWords whiteSpace xs
                             | otherwise = (word:(splitWords whiteSpace current))
                              where (word, current) = break whiteSpace (x:xs) 
...
mostFrequent n xs = foldr (\x y -> x ++ if y =="" then y else " " ++ y) "" (reverse words)
                      where words = take n (map (\(x,y) -> y) sortedFreqs)
                            sortedFreqs = reverse (sort frequencies)
                            frequencies = getFreqs (getDivision (sort (splitWords whiteSpace (map (\x -> toLower x) xs))))

getDivision :: [String] -> [[String]]
getDivision xs = group xs

getFreqs :: [[String]] -> [(Int, String)]
getFreqs xs = map (\x -> (length x, head x)) xs
