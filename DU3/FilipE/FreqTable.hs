...
mostFrequent n s = do_stringu
					where 
						ret = map (toLower) s
						pole = splitWords whiteSpace ret
						vyskyty = vyskyt pole
						podla_vyskytu = reverse (sort vyskyty)
						vyzadovane = take n podla_vyskytu
						slova = map (snd) vyzadovane
						do_stringu = intercalate " " (reverse slova)

-- vyuzitie funkcii z predchadzajucich uloh :D
splitWords  :: (Char -> Bool) -> String -> [String]
splitWords f "" = []
splitWords f s = filter (/= "") $ fst dvojica ++ [snd dvojica] 
					where 
						dvojica = foldl (\(a,b) -> \x -> pomocna f a b x  ) ([],"") s 


pomocna :: (Char -> Bool) -> [String] -> String -> Char -> ([String],String)
pomocna f p s z = if f z then (p ++ [s], "")
				  else (p,s++[z])
...
vyskyt :: [String] -> [(Int, String)]
vyskyt [] = []
vyskyt xs = nub $ zip [pocty x xs| x <- xs] xs 


pocty :: Eq a => a -> [a] -> Int
pocty x =  length . filter (==x)
